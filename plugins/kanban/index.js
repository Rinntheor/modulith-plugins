...
//
// 看板 —— 把活儿按列表摆开，拖动或按键盘挪动卡片。另有一页「速记」用于随手记下再拆成卡片。
//
// 手写 IIFE，不使用构建工具，因此不能写 JSX，全部用 `React.createElement`（简写为 h）。
//
// 用到三项权限，与代码里的调用一一对应：
//   storage             —— 看板数据、速记草稿、提醒开关都写在插件自己的存储里
//   notification        —— 到期 / 逾期提醒与「清除已完成」这类操作的结果提示
//   plugin-communicate  —— 分屏时两个实例互相告知「这边改过了」，避免后写的一方覆盖先写的
// 没有用到任何原生能力（launcher / icons / shell / fileDrop / audio），因此一项都不声明。
//
// 两处实现上的取舍，写在这里免得被误认成疏忽：
//
// 1. 拖放是自己用 Pointer Events 实现的。宿主为了接收系统拖入而开启了 Tauri 的拖放拦截，
//    WebView 内的 HTML5 拖放（dragstart / drop）整体不可用。卡片与列表都能拖，
//    并且都配有等价的键盘 / 菜单操作。
//
// 2. 卡片在列表里的显示顺序按「未完成优先 → 优先级 → 截止日 → 手动顺序」排。
//    因此手动拖动只在同一优先级与截止日的卡片之间改变顺序 —— 这是刻意的：
//    如果可以把「低优先级」拖到「高优先级」上面，那优先级字段就形同虚设了。
//    落点会按这个排序规则换算成真实位置（用邻卡 id 而不是可见下标），
//    所以松手后卡片一定停在指示线指的那个位置。
...
(function () {
  ...
  if (!Modulith) {
    console.error('[kanban] 未找到 window.Modulith，插件无法加载');
    return;
  }
  ...
  var DRAFT_MAX = 4000;
  var LANE_AUTO_DONE = '完成'; // 名字里带这两个字的列表，拖进去自动标记为已完成
  var UNDO_MS = 8000;
